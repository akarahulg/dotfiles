...
local function worker(user_args)
    ...
    -- Store user_args in the calendar_widget table
    calendar_widget.user_args = user_args
    ...
    local theme = user_args.theme or 'naughty'
    local placement = user_args.placement or 'top'
    local radius = user_args.radius or 8
    local next_month_button = user_args.next_month_button or 4
    local previous_month_button = user_args.previous_month_button or 5
    local start_sunday = user_args.start_sunday or false
    local week_numbers = user_args.week_numbers or false
    ...
    function calendar_widget.toggle()
        local auto_hide = calendar_widget.user_args.auto_hide
        local timeout = calendar_widget.user_args.timeout or 2
        ...
        if popup.visible then
            ...
            elseif placement == 'bottom' then
                awful.placement.bottom(popup, { margins = { bottom = 30}, parent = awful.screen.focused() })
            else
                awful.placement.top(popup, { margins = { top = 30 }, parent = awful.screen.focused() })
            end
            ...
            popup.visible = true
            if auto_hide then
                auto_hide_timer.timeout = timeout
